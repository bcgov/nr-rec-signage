    ...
    <div ref={bannerRef} className="blade-sign w-100 d-flex flex-column align-items-center justify-content-center">
        ...
        >
            <div style={{backgroundColor: '#4E3629', borderRadius: `${inch * 0.5}px`,width: '100%',height: '100%',display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                <p  ref={blade1Ref} style={{lineHeight: 1,textTransform: 'uppercase',letterSpacing: `${inch * 0.5}px`,fontSize: `${initialFontSize *  parseFloat(fields.get('resizer_blade_1')?.value || '100')/100 }px`,color: 'white', textAlign: 'center'}}>{fields.get('blade_1_text')?.value || 'Not set'}</p>
            </div>
        </div>
        ...
        >
            <p style={{textTransform: 'uppercase',letterSpacing: `${inch * 0.5}px`,fontSize: `${initialFontSize *  parseFloat(fields.get('resizer_blade_2')?.value || '100')/100 }px`,color: 'white', textAlign: 'center'}}>{fields.get('blade_2_text')?.value || 'Not set'}</p>
        </div>
